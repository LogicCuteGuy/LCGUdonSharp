...
- `AsyncImageDownloadExample` awaits `VRCAsync.LoadImageAsync` while keeping
  the traditional image callbacks, and calls ordinary `ref`/`out` helpers both
...
Ordinary synchronous U# methods may use `ref` and `out` with locals, fields,
and one-dimensional array elements. In the current async slice, calls to those
helpers may appear before or after an SDK await when their arguments are fields
or one-dimensional array elements; `AsyncImageDownloadExample` demonstrates
both copy-back forms. Async locals are not hoisted yet. C# also does not permit
`ref`, `in`, or `out` parameters on the async method itself, so keep those
parameters on synchronous helper methods.
