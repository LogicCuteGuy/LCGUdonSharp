    ...
    internal abstract class TypeSymbol : Symbol
    {
        ...
        public override void Bind(BindContext context)
        {
            ...
            foreach (ISymbol member in members.Where(member => (!member.IsImplicitlyDeclared || member.Kind == SymbolKind.Field)))
            {
                ...
                {
                    case IFieldSymbol _:
                    case IPropertySymbol property when !property.IsStatic && (IsUdonSharpBehaviour || IsUdonSharpInterface):
                    case IMethodSymbol method when !method.IsStatic && (IsUdonSharpBehaviour || IsUdonSharpInterface):
        ...
        private void ValidateImplementedInterfaces()
        {
            INamedTypeSymbol behaviourType = (INamedTypeSymbol)RoslynSymbol;

            foreach (INamedTypeSymbol interfaceType in behaviourType.AllInterfaces)
            {
                if (interfaceType.IsExternType())
                    continue;
            ...
            foreach (IMethodSymbol method in behaviourType.GetMembers().OfType<IMethodSymbol>())
            {
